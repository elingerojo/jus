...
module.exports = function jus (sourceDir, targetDir) {
  ...
  function createFile(filename) {
    ...
    if (fileTypes.datafile.test(filename)) type = 'datafile'
  ...
  function updateFile(filename) {
    ...
    // debounce extra dupicated chokidar 'change' events
    if (file.inUpdateQueue) {
//      console.log('&&&&&&&&&&&&&&&&&&&&&& bounced filename =', filename)
      return
    }
    file.inUpdateQueue = true
// file.squeeze() moved to server.js so be decontextualized first
//    file.squeeze()
