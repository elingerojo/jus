...
//  console.log('files.length = ', files.length)
  ...
  files.forEach(file => {
    ...
    // --- Only for 'page' type ---
    // Used for a special feature that auto includes all loose pages into
    // arrays to be rendered as lists using handlebars #each blocks
    // Following is the feature "add loose pages into arrays" first step:
    // Make two globals lists of keys and files depending wheather the keys
    // are array or not.
    if (file.type === 'page') {
...
//    page.data = {}
  ...
  // Add "not-arried" keys to pages that have "arried" keys
  //
//  console.log('2 -----------------------------------------------')
...
//              console.log('2 ---- ---- ---- ---- ---- ---- also added, now new naPage.autoLinkedReferences.length = ', naPage.autoLinkedReferences.length)
  ...
  // Feature: "indirect data"
  // load indirect data _jus_indirect_ so it can be contextualized
  //
  // Indirect key reserved word for feature "indirect data"
  const indirectKey = '_jus_indirect_'
