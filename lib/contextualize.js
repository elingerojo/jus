...
//          console.log('*-*-*-**-* gKey NO existia')
        }
        notArrayGlobalKeys[gKey].push(file)
//        console.log('*-*-*-**-* notArrayGlobalKeys[gKey].length = ', notArrayGlobalKeys[gKey].length)
      })
//      console.log('^')
    }
...
//    page.data = {}
    ...
    context.partials.forEach(partial => {
      // only descendant partials
      if (partial.path.dir.includes(page.path.dir)) {
...
//          console.log('lp[gKey] = ', lp[gKey])
          lp.autoLinkedReference.push({page: page, gKey: gKey})
