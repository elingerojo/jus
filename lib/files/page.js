...
//    console.log('data = ', data)
  ...
  getBackmatter() {
    ...
    if (lastComment) {
      ...
      var output = marky(commentContent, {
...
//      output = output.html() // old marky-markdown "api"
...
//    $ = cheerio.load(output)
    if (this.isMarkdown) {
      // Compute and insert Table of Content between toc comment tags, if any
      output = toc.insert(output)
      this.$ = cheerio.load(marky(output, { // marky-markdown 9.0 new "api"
        sanitize: false,            // allow script tags and stuff
        linkify: true,              // turn orphan URLs into hyperlinks
        highlightSyntax: true,      // run highlights on fenced code blocks
        prefixHeadingIds: false,    // prevent DOM id collisions
